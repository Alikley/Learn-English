...
export function BoxCard({
  ...
  async function handleDeleteBox() {
    if (
      box.wordCount > 0 &&
      !window.confirm(
        `جعبه «${box.name}» و ${box.wordCount} کلمه‌ای داخلش حذف شود؟`,
      )
    )
      return;
      ...
      <div className="flex-1 space-y-1.5 min-h-[3rem]">
        {box.words.length === 0 ? (
          <p className="text-[11px] text-slate-300 leading-5 py-2">
            هنوز کلمه‌ای نیست — روی کلمه‌های انگلیسی سایت هاور کن و «جعبه لغت»
            را بزن، یا همین‌جا بنویس
          </p>
        ) : (
          box.words.map((w) => (
            <div
      ...
      <div className="mt-3 pt-3 border-t border-slate-100">
        {isFull ? (
          <p className="text-[11px] text-red-400 text-center py-1.5">
            جعبه پر است — اول کلمه‌ای را حذف کن
          </p>
        ) : (
...
export function EmptyBoxesHint() {
  return (
    <div className="col-span-full flex flex-col items-center gap-2 py-10 text-slate-300">
      ...
      <p className="text-sm font-bold">جعبه‌ای نیست — بالای صفحه جعبه بساز</p>
    </div>
  );
}
