...
//
// v1.0.2.7 — ریفکتوری: امتیاز پاسخ/دور بی‌نقص و چرخهٔ سطح
// به speedQuizLogic.ts منتقل شد.
// ========================================
...
export function useSpeedQuizGame({
  ...
  const [timeLeftMs, setTimeLeftMs] = useState<number>(QUESTION_TOTAL_MS);
  ...
  const startGame = useCallback(
    async (lvl: GameLevel, opts?: { hideLevel?: boolean }) => {
      ...
      setFeedback(null);
      setLastGained(0);
      setTimeLeftMs(QUESTION_TOTAL_MS);
      setOutcome(null);
      ...
      try {
        const res = await fetch(speedQuizQuestionsUrl(lvl));
        if (!res.ok) {
          setPhase("error");
          return;
        }
  ...
  const handleAnswer = useCallback(
    (index: number | null) => {
      ...
      const gained = correct ? correctAnswerGain(timeLeftMs, combo) : 0;
        ...
        window.setTimeout(
          () => {
            ...
            if (isLast || outOfLives) {
              const finalScore = finalSessionScore(newScore, newWrong, newLives);
              setScore(finalScore);
              // نتیجه نهایی دور (گام ۴):
  ...
  const handleNextLevel = useCallback(() => {
    void startGame(nextLevelOf(level), { hideLevel: true });
  }, [level, startGame]);
