// ============================================================
// لغت‌نامهٔ محلی (آفلاین) — فقط سمت سرور
// ادغام واژگان درس‌های سایت + بازی حافظه + کوییز سرعتی
// اولویت هنگام تداخل: درس‌ها > حافظه > کوییز سرعتی
// بازتولید: scripts/gen-local-dictionary.py — دستی ویرایش نشود
// ============================================================

const LOCAL_ENTRIES: Record<string, string> = {
  "a piece of cake": "مثل آب خوردن",
  "abroad": "به خارج، خارج از کشور",
  "abstract": "انتزاعی",
  "abundant": "فراوان",
  "accept": "پذیرفتن",
  "accomplish": "به سرانجام رساندن",
  "account": "حساب (بانکی)",
  "achieve": "به دست آوردن",
  "achievement": "دستاورد",
  "adventure": "ماجراجویی",
  "advertisement": "تبلیغات",
  "advise": "توصیه کردن",
  "afford": "توان خرید داشتن",
  "afternoon": "بعدازظهر",
  "airport": "فرودگاه",
  "allergy": "حساسیت، آلرژی",
  "ambiguous": "مبهم",
  "ambitious": "جاه‌طلب",
  "analyze": "تحلیل کردن",
  "anniversary": "سالگرد",
  "answer": "پاسخ / پاسخ دادن",
  "ant": "مورچه",
  "anxiety": "اضطراب",
  "anxious": "مضطرب، دلواپس",
  "apologize": "عذرخواهی کردن",
  "apparent": "بدیهی",
  "appeal": "تجدیدنظرخواهی",
  "apple": "سیب",
  "apply for": "درخواست دادن (شغل)",
  "appointment": "نوبت، وقت قبلی",
  "approach": "رویکرد",
  "appropriate": "مناسب",
  "approve": "تأیید کردن",
  "architect": "معمار",
  "argue": "بحث کردن",
  "arm": "بازو",
  "arrive": "رسیدن",
  "article": "مقاله، خبر",
  "artificial": "مصنوعی",
  "artificial intelligence": "هوش مصنوعی",
  "assess": "سنجیدن",
  "assignment": "تکلیف (دانشگاهی)",
  "assumption": "پیش‌فرض",
  "astronaut": "فضانورد",
  "atom": "اتم",
  "attitude": "نگرش",
  "attract": "جذب کردن",
  "aunt": "عمه / خاله",
  "authentic": "حقیقی",
  "autumn": "پاییز",
  "available": "در دسترس",
  "baby": "نوزاد",
  "balcony": "بالکن",
  "banana": "موز",
  "barrier": "سد",
  "basketball": "بسکتبال",
  "bathroom": "دستشویی / حمام",
  "bear": "خرس",
  "beautiful": "زیبا",
  "bed": "تخت",
  "bedroom": "اتاق خواب",
  "bee": "زنبور",
  "betray": "خیانت کردن",
  "big": "بزرگ",
  "bird": "پرنده",
  "bite the bullet": "دندان روی جگر گذاشتن",
  "bitterly cold": "تلخ‌سرد (شدیداً سرد)",
  "black": "سیاه",
  "blackboard": "تخته سیاه",
  "blue": "آبی",
  "boarding pass": "کارت سوار شدن",
  "bone": "استخوان",
  "book": "کتاب",
  "book (a room)": "رزرو کردن",
  "boot": "چکمه",
  "borrow": "قرض گرفتن",
  "boy": "پسر",
  "bread": "نان",
  "break the ice": "یخ مجلس را شکستن",
  "breakfast": "صبحانه",
  "breaking news": "خبر فوری",
  "breakthrough": "پیشرفت چشمگیر",
  "broadcast": "پخش کردن / پخش",
  "brother": "برادر",
  "budget": "بودجه / بودجه‌بندی کردن",
  "butter": "کره",
  "butterfly": "پروانه",
  "cabinet": "کابینه",
  "cake": "کیک",
  "calculator": "ماشین‌حساب",
  "calendar": "تقویم",
  "camel": "شتر",
  "cancel": "لغو کردن",
  "candidate": "نامزد",
  "candy": "آب‌نبات",
  "capacity": "ظرفیت",
  "carrot": "هویج",
  "carry": "حمل کردن",
  "cat": "گربه",
  "cause": "باعث شدن",
  "celebration": "جشن",
  "chair": "صندلی",
  "chalk": "گچ",
  "challenge": "چالش",
  "championship": "قهرمانی",
  "character": "شخصیت (داستان)",
  "charge": "شارژ کردن / شارژ",
  "cheap": "ارزان",
  "check in": "ثبت‌نام ورود (هتل/پرواز)",
  "cheese": "پنیر",
  "cherry": "آلبالو",
  "chicken": "مرغ",
  "chocolate": "شکلات",
  "choose": "انتخاب کردن",
  "chronic": "مزمن",
  "circumstance": "شرایط",
  "classroom": "کلاس (اتاق)",
  "clause": "بند (قرارداد)",
  "climate change": "تغییرات اقلیمی",
  "climb": "بالا رفتن",
  "closely related": "به‌شدت مرتبط",
  "cloud": "ابر",
  "cloudy": "ابری",
  "coach": "مربی",
  "coat": "کت (زمستانی)",
  "coffee": "قهوه",
  "coherent": "منسجم",
  "coincidence": "هم‌زمانی",
  "cold": "سرد",
  "collaborate": "همکاری کردن",
  "collapse": "فروریختن",
  "colleague": "همکار",
  "come": "آمدن",
  "commit": "مرتکب شدن",
  "compare": "مقایسه کردن",
  "compelling": "گیرا، قانع‌کننده",
  "compete": "رقابت کردن",
  "competition": "رقابت",
  "complain": "شکایت کردن",
  "composition": "ترکیب‌بندی",
  "comprehend": "درک کردن",
  "comprehensive": "جامع",
  "confident": "با اعتماد به نفس",
  "confront": "مواجه شدن",
  "congratulate": "تبریک گفتن",
  "conscience": "وجدان",
  "consequence": "پیامد",
  "consistent": "سازگار، همخوان",
  "constitution": "قانون اساسی",
  "consult": "مشورت کردن",
  "contemporary": "معاصر",
  "contract": "قرارداد",
  "controversy": "مناقشه",
  "convince": "متقاعد کردن",
  "cookie": "کوکی",
  "correspond": "مکاتبه کردن",
  "cost an arm and a leg": "خیلی گران بودن",
  "cousin": "پسرعمو",
  "coverage": "پوشش خبری",
  "cow": "گاو",
  "create": "ایجاد کردن",
  "crocodile": "کروکودیل",
  "crowded": "شلوغ",
  "crucial": "حیاتی",
  "culture": "فرهنگ",
  "curious": "کنجکاو",
  "currency": "ارز، واحد پول",
  "custody": "حضانت / بازداشت",
  "dad": "پدر",
  "dangerous": "خطرناک",
  "data": "داده‌ها",
  "daughter": "دختر (فرزند)",
  "day": "روز",
  "deadline": "مهلت (تحویل کار)",
  "debt": "بدهی",
  "deceive": "فریب دادن",
  "decide": "تصمیم گرفتن",
  "decline": "افول کردن",
  "dedicate": "وقف کردن",
  "deeply moving": "عمیقاً تأثیرگذار",
  "deer": "آهو",
  "defeat": "شست / مغلوب کردن",
  "defendant": "متهم",
  "degree": "مدرک دانشگاهی",
  "delay": "تأخیر",
  "delicious": "خوشمزه",
  "deliver": "تحویل دادن",
  "demonstrate": "نشان دادن",
  "dentist": "دندانپزشک",
  "deny": "انکار کردن",
  "depend": "وابسته بودن",
  "derive": "استخراج/برخاستن",
  "desert": "بیابان",
  "desk": "میز تحریر",
  "desperate": "ناامید",
  "destination": "مقصد",
  "destroy": "نابود کردن",
  "determine": "تعیین کردن",
  "develop": "توسعه دادن",
  "device": "دستگاه، وسیله",
  "diagnosis": "تشخیص (بیماری)",
  "dictionary": "فرهنگ لغت",
  "different": "متفاوت",
  "difficult": "سخت",
  "dilemma": "دوراهی",
  "diminish": "کوچک شدن",
  "diplomacy": "دیپلماسی",
  "disappointed": "ناامید، مأیوس",
  "disclose": "افشا کردن",
  "discover": "کشف کردن",
  "dishwasher": "ماشین ظرف‌شویی",
  "dismiss": "برکنار کردن",
  "distinguish": "تمایز قائل شدن",
  "distribute": "توزیع کردن",
  "diverse": "متنوع",
  "divide": "تقسیم کردن",
  "doctor": "پزشک",
  "dog": "سگ",
  "dolphin": "دلفین",
  "dominate": "سلطه داشتن",
  "donkey": "الاغ",
  "door": "در، دری",
  "doubt": "شک",
  "download": "دانلود کردن",
  "dramatic": "دراماتیک",
  "dress": "لباس زنانه",
  "drink": "نوشیدن",
  "driver": "راننده",
  "drought": "خشکسالی",
  "duck": "اردک",
  "ear": "گوش",
  "earn": "کسب کردن",
  "earthquake": "زلزله",
  "eat": "خوردن",
  "education": "آموزش",
  "efficient": "کارآمد",
  "egg": "تخم‌مرغ",
  "election": "انتخابات",
  "electricity": "برق",
  "elephant": "فیل",
  "eliminate": "از میان بردن",
  "embarrassed": "خجالت‌زده",
  "embassy": "سفارت",
  "emerge": "ظاهر شدن",
  "emergency": "اورژانس، وضعیت اضطراری",
  "emphasize": "تأکید کردن",
  "employee": "کارمند",
  "employer": "کارفرما",
  "endangered": "در خطر انقراض",
  "endeavor": "کوشش",
  "energy": "انرژی",
  "enhance": "ارتقا دادن",
  "enjoy": "لذت بردن",
  "ensure": "اطمینان دادن",
  "equivalent": "معادل",
  "eraser": "پاک‌کن",
  "escape": "فرار کردن",
  "establish": "برقرار کردن",
  "estimate": "تخمین زدن",
  "evaluate": "ارزیابی کردن",
  "evening": "عصر",
  "evidence": "شواهد، دلیل",
  "evidence (legal)": "دلیل و مدرک (قضایی)",
  "evolution": "فرگشت، تکامل",
  "evolve": "تکامل یافتن",
  "exaggerate": "بزرگ‌نمایی کردن",
  "exam": "امتحان",
  "examine": "بررسی کردن",
  "exceed": "فراتر رفتن",
  "exercise": "ورزش کردن",
  "exhibition": "نمایشگاه",
  "expect": "انتظار داشتن",
  "expense": "هزینه",
  "expensive": "گران",
  "experience": "تجربه (کاری)",
  "experiment": "آزمایش",
  "explain": "توضیح دادن",
  "explicit": "صریح",
  "explore": "کاوش کردن",
  "extensive": "گسترده",
  "extraordinary": "خارق‌العاده",
  "eye": "چشم",
  "eyebrow": "ابرو",
  "facilitate": "تسهیل کردن",
  "family": "خانواده",
  "farmer": "کشاورز",
  "father": "پدر",
  "fatigue": "فرسودگی",
  "finger": "انگشت",
  "fire": "آتش",
  "fireworks": "آتش‌بازی",
  "fish": "ماهی",
  "fit (healthy)": "در فرم، آماده جسمانی",
  "flexible": "منعطف",
  "flight": "پرواز",
  "flood": "سیل",
  "flower": "گل",
  "fluctuate": "نوسان داشتن",
  "foot": "پا",
  "forbid": "ممنوع کردن",
  "forehead": "پیشانی",
  "forest": "جنگل",
  "forgive": "بخشیدن",
  "fork": "چنگال",
  "fox": "روباه",
  "framework": "چارچوب",
  "friendship": "دوستی",
  "frog": "قورباغه",
  "fruit": "میوه",
  "fully aware": "کاملاً واقف",
  "furious": "خشمگین",
  "gallery": "گالری",
  "garden": "باغچه / باغ",
  "gather": "گردآوردن",
  "genome": "ژنوم",
  "genuine": "اصیل",
  "gift": "هدیه",
  "giraffe": "زرافه",
  "girl": "دختر",
  "gloves": "دستکش",
  "go": "رفتن",
  "goal": "گل / هدف",
  "goat": "بز",
  "golden": "طلایی",
  "gradual": "تدریجی",
  "graduation": "دانش‌آموختگی",
  "grandfather": "پدربزرگ",
  "grandmother": "مادربزرگ",
  "grape": "انگور",
  "grasshopper": "ملخ",
  "grateful": "ممنون، قدردان",
  "gravity": "گرانش",
  "green": "سبز",
  "growth": "رشد",
  "guarantee": "تضمین",
  "hair": "مو",
  "hand": "دست",
  "happy": "خوشحال",
  "hat": "کلاه",
  "head": "سر",
  "headline": "تیتر، عنوان خبر",
  "headphones": "هدفون",
  "healthy": "سالم",
  "heart": "قلب",
  "heavy rain": "باران شدید",
  "hesitant": "مردد",
  "hesitate": "تردید کردن",
  "highly recommended": "به‌شدت توصیه‌شده",
  "highway": "بزرگراه",
  "history": "تاریخ",
  "hit the books": "درس خواندن جدی",
  "homework": "تکلیف",
  "honey": "عسل",
  "horse": "اسب",
  "hot": "داغ",
  "hour": "ساعت",
  "house": "خانه",
  "hundred": "صد",
  "hypothesis": "فرضیه",
  "imagery": "تصویرسازی",
  "imagine": "تصور کردن",
  "immune system": "سیستم ایمنی",
  "important": "مهم",
  "improve": "بهبود دادن",
  "in shape": "در فرم",
  "include": "شامل بودن",
  "increase": "افزایش دادن",
  "inevitable": "اجتناب‌ناپذیر",
  "inflation": "تورم",
  "influential": "تأثیرگذار",
  "ingredient": "ماده اولیه",
  "injury": "آسیب، جراحت",
  "innovation": "نوآوری",
  "insect": "حشره",
  "insight": "بینش",
  "insurance": "بیمه (درمانی)",
  "interest rate": "نرخ بهره",
  "interesting": "جالب",
  "invest": "سرمایه‌گذاری کردن",
  "invitation": "دعوت‌نامه",
  "invite": "دعوت کردن",
  "island": "جزیره",
  "jacket": "ژاکت",
  "jealous": "حسود",
  "join": "پیوستن",
  "journalist": "خبرنگار",
  "journey": "سفر طولانی",
  "juice": "آبمیوه",
  "justify": "توجیه کردن",
  "key": "کلید",
  "kitchen": "آشپزخانه",
  "knee": "زانو",
  "knowledge": "دانش",
  "laboratory": "آزمایشگاه",
  "lake": "دریاچه",
  "lamp": "چراغ",
  "landscape": "منظره (نقاشی)",
  "leaf": "برگ",
  "lecture": "کلاس نظری، سخنرانی",
  "legislation": "قانون‌گذاری، مصوبات",
  "lemon": "لیمو",
  "lend": "قرض دادن",
  "let sleeping dogs lie": "پینهدان را نچیدن",
  "liable": "مسئول (قانونی)",
  "lightning": "رعدوبرق",
  "lion": "شیر",
  "lip": "لب",
  "living room": "پذیرایی",
  "loan": "وام",
  "lonely": "تنها و دل‌تنگ",
  "luggage": "چمدان و اثاث سفر",
  "maintain": "حفظ کردن",
  "make a decision": "تصمیم گرفتن",
  "manager": "مدیر",
  "market": "بازار",
  "masterpiece": "شاهکار",
  "match": "مسابقه",
  "measure": "اندازه گرفتن",
  "meat": "گوشت",
  "medicine": "دارو",
  "meeting": "جلسه",
  "mention": "اشاره کردن",
  "metaphor": "استعاره",
  "methodology": "روش‌شناسی",
  "meticulous": "بسیار دقیق و موشکاف",
  "microwave": "مایکروویو",
  "milk": "شیر",
  "minute": "دقیقه",
  "mirror": "آینه",
  "molecule": "مولکول",
  "mom": "مادر",
  "monkey": "میمون",
  "moon": "ماه",
  "morning": "صبح",
  "mother": "مادر",
  "motivate": "انگیزه دادن",
  "mountain": "کوه",
  "mouse": "موش",
  "museum": "موزه",
  "narrator": "راوی",
  "negligible": "ناچیز",
  "negotiate": "مذاکره کردن",
  "negotiation": "مذاکره",
  "neighborhood": "محله",
  "nervous": "مضطرب",
  "newspaper": "روزنامه",
  "night": "شب",
  "noon": "ظهر",
  "nose": "بینی",
  "notebook": "دفتر",
  "obstacle": "مانع",
  "offer": "پیشنهاد دادن",
  "once in a blue moon": "به‌ندرت، هر از گاهی",
  "one / two / three": "یک / دو / سه",
  "one-way ticket": "بلیط یک‌طرفه",
  "onion": "پیاز",
  "online": "آنلاین، برخط",
  "opportunity": "فرصت",
  "orange": "پرتقال",
  "outcome": "برآیند، نتیجه",
  "overwhelmed": "درمانده، تحت فشار شدید",
  "page": "صفحه",
  "parent": "والدین",
  "parrot": "طوطی",
  "particle": "ذره",
  "passenger": "مسافر",
  "password": "رمز عبور",
  "pay attention": "توجه کردن",
  "peach": "هلو",
  "pen": "خودکار",
  "pencil": "مداد",
  "penguin": "پنگوئن",
  "pepper": "فلفل",
  "perspective": "پرسپکتیو، منظر",
  "pharmacy": "داروخانه",
  "phenomenon": "پدیده",
  "pig": "خوک",
  "pink": "صورتی",
  "plate": "بشقاب",
  "plot": "خط داستان، پیرنگ",
  "plumber": "لوله‌کش",
  "pocket": "جیب",
  "poetic": "شاعرانه",
  "policy": "سیاست (عمومی)",
  "pollution": "آلودگی",
  "popcorn": "پاپ‌کورن",
  "portrait": "پرتره",
  "postgraduate": "تحصیلات تکمیلی",
  "precise": "دقیق",
  "prejudice": "تعصب",
  "prepare": "آماده کردن",
  "prescribe": "تجویز کردن",
  "prescription": "نسخه (دارو)",
  "president": "رئیس‌جمهور",
  "prevent": "جلوگیری کردن",
  "prevention": "پیشگیری",
  "price": "قیمت",
  "priority": "اولویت",
  "professor": "پروفسور",
  "profound": "عمیق",
  "promise": "قول دادن",
  "promotion": "ارتقا",
  "protect": "حفاظت کردن",
  "proud": "مفتخر",
  "purple": "بنفش",
  "qualification": "صلاحیت، مدرک حرفه‌ای",
  "question": "سؤال",
  "quiet": "ساکت",
  "rabbit": "خرگوش",
  "rain": "باران",
  "rainbow": "رنگین‌کمان",
  "rainy": "بارانی",
  "read": "خواندن",
  "realize": "درک کردن",
  "receive": "دریافت کردن",
  "recession": "رکود",
  "recipe": "دستور پخت",
  "recovery": "بهبودی",
  "recycle": "بازیافت کردن",
  "red": "قرمز",
  "reduce": "کاهش دادن",
  "referendum": "همه‌پرسی",
  "refrigerator": "یخچال",
  "refuse": "رد کردن",
  "rehabilitation": "توانبخشی",
  "relieved": "آسوده‌خاطر",
  "reluctant": "بی‌میله",
  "remarkable": "چشمگیر",
  "remind": "یادآوری کردن",
  "renewable": "تجدیدپذیر",
  "rent": "اجاره کردن",
  "repair": "تعمیر کردن",
  "reporter": "خبرنگار",
  "research": "پژوهش، تحقیق",
  "resign": "استعفا دادن",
  "resilient": "تاب‌آور",
  "restaurant": "رستوران",
  "revenue": "درآمد (کلان)",
  "rice": "برنج",
  "rigorous": "دقیق و اصولی",
  "room": "اتاق",
  "ruler": "خط‌کش",
  "rumour": "شایعه",
  "run": "دویدن",
  "sad": "غمگین",
  "salary": "حقوق (ماهانه)",
  "salt": "نمک",
  "sanction": "تحریم",
  "sand": "شن",
  "sandwich": "ساندویچ",
  "save (money)": "پس‌انداز کردن",
  "scarce": "کمیاب",
  "scarf": "شال، روسری",
  "scholarship": "بورس تحصیلی",
  "school": "مدرسه",
  "science": "علوم",
  "scientist": "دانشمند",
  "score": "امتیاز / امتیاز گرفتن",
  "screen": "صفحه نمایش",
  "scrutinize": "با دقت بررسی کردن",
  "sculpture": "مجسمه",
  "separate": "جدا کردن",
  "setting": "زمان و مکان داستان",
  "sheep": "گوسفند",
  "shirt": "پیراهن (ساده)",
  "shoes": "کفش",
  "shoulder": "شانه",
  "sick": "مریض",
  "sightseeing": "گشت و دیدن جاهای دیدنی",
  "significant": "چشمگیر، معنادار",
  "silver": "نقره‌ای",
  "singer": "خواننده",
  "sister": "خواهر",
  "skeptical": "شکاک",
  "sleep": "خوابیدن",
  "small": "کوچک",
  "smartphone": "گوشی هوشمند",
  "snake": "مار",
  "snow": "برف",
  "snowy": "برفی",
  "social media": "شبکه‌های اجتماعی",
  "sock": "جوراب",
  "software": "نرم‌افزار",
  "solve": "حل کردن",
  "son": "پسر (فرزند)",
  "soup": "سوپ",
  "source": "منبع",
  "spend": "خرج کردن",
  "spider": "عنکبوت",
  "spill the beans": "راز را لو دادن",
  "spoon": "قاشق",
  "spring": "بهار",
  "star": "ستاره",
  "stomach": "معده",
  "strong coffee": "قهوه پررنگ",
  "student": "دانش‌آموز / دانشجو",
  "study": "درس خواندن",
  "subsidy": "یارانه",
  "sufficient": "کافی",
  "sugar": "شکر",
  "summarize": "خلاصه کردن",
  "summer": "تابستان",
  "sun": "خورشید",
  "sunglasses": "عینک آفتابی",
  "sunny": "آفتابی",
  "supply and demand": "عرضه و تقاضا",
  "surgery": "جراحی",
  "sustainable": "پایدار",
  "sympathetic": "همدرد",
  "symptom": "علامت (بیماری)",
  "table": "میز",
  "tailor": "خیاط",
  "take a risk": "ریسک کردن",
  "tea": "چای",
  "teacher": "معلم",
  "technology": "فناوری",
  "telephone": "تلفن",
  "television": "تلویزیون",
  "temperature": "دما",
  "ten": "ده",
  "the ball is in your court": "توپ در زمین توست",
  "theme": "مضمون",
  "theory": "نظریه",
  "therapy": "درمان‌درمانی",
  "thorough": "کامل و جامع",
  "ticket": "بلیط",
  "tiger": "ببر",
  "tomato": "گوجه",
  "tomorrow": "فردا",
  "toothbrush": "مسواک",
  "toothpaste": "خمیردندان",
  "trade": "تجارت، مبادله",
  "traffic": "ترافیک",
  "training": "تمرین، آماده‌سازی",
  "transform": "دگرگون کردن",
  "treatment": "درمان",
  "treaty": "پیمان، معاهده",
  "tree": "درخت",
  "trousers": "شلوار",
  "tuition fees": "شهریه",
  "turtle": "لاک‌پشت",
  "twenty": "بیست",
  "uncle": "عمو / دایی",
  "under the weather": "کمی ناخوش",
  "undergraduate": "کارشناسی",
  "undermine": "تضعیف کردن",
  "understanding": "درک",
  "unemployment": "بیکاری",
  "university": "دانشگاه",
  "update": "به‌روزرسانی",
  "vaccine": "واکسن",
  "vegetables": "سبزیجات",
  "versatile": "همه‌فن‌حریف",
  "violate": "نقض کردن",
  "vital signs": "علائم حیاتی",
  "vivid": "زنده، پرجزئیات",
  "volcano": "آتشفشان",
  "volleyball": "والیبال",
  "wake up": "بیدار شدن",
  "warm up": "گرم کردن (بدن)",
  "waste": "پسماند / هدر دادن",
  "water": "آب",
  "waterfall": "آبشار",
  "weather": "آب‌وهوا",
  "week": "هفته",
  "weekend": "آخر هفته",
  "white": "سفید",
  "window": "پنجره",
  "windy": "بادی",
  "winter": "زمستان",
  "wireless": "بی‌سیم",
  "witness": "شاهد",
  "wolf": "گرگ",
  "wonderful": "شگفت‌انگیز",
  "work": "کار کردن",
  "write": "نوشتن",
  "year": "سال",
  "yellow": "زرد",
  "yesterday": "دیروز",
  "yogurt": "ماست",
};

const LOOKUP: Map<string, string> = new Map(
  Object.entries(LOCAL_ENTRIES).map(([w, fa]) => [w.toLowerCase(), fa]),
);

/** جست‌وجوی معنی فارسی یک کلمهٔ انگلیسی در لغت‌نامهٔ محلی — بدون نیاز به اینترنت/AI */
export function lookupLocalTranslation(word: string): string | null {
  const key = word.trim().toLowerCase();
  if (!key) return null;
  return LOOKUP.get(key) ?? null;
}

/** تعداد مدخل‌های لغت‌نامهٔ محلی */
export const localDictionarySize: number = LOOKUP.size;
